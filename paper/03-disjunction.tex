...
\end{code}
That is, we implement \hask{Record} as a heterogeneous linked-list\footnotemark.
\footnotetext{%
...
\end{code}
\hask{FindIndex'} is a variant of \hask{FindIndex}, which returns a raw membership if it is found, and throwing a type-error otherwise.
...
In expressing the existence of witnesses, it allows much precise and robust handling to use the \hask{Given} class.
