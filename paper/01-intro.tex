...
Briefly speaking, a singleton type \hask{Sing a} of a expression-level value \hask{a} is a unique type that has the same structure as \hask{a}, on which we can pattern-match to retrieve the exact shape of it. \hask{Sing a} can be identified with \hask{a} but promoted to type-level.
