...
\begin{itemize}
  \item In \Cref{sec:gcd}, taking a type-level GCD as an example, we demonstrate how to \emph{demote} closed type-level functions involving type-level pattern-matching.
  ...
  \item \Cref{sec:plugins} is devoted to a practical example of a dependently-typed plugin system that can be dynamically type-checked at runtime.
...
Briefly speaking, a singleton type \hask{Sing a} of an expression-level value \hask{a} is the unique type that has the same structure as \hask{a}, on which we can pattern-match to retrieve the exact shape of it. \hask{Sing a} can be identified with \hask{a} but promoted to type-level.
...
\begin{enumerate}
\item We prefix with the capital \hask{S} for singleton types: for example, \hask{SNat} is the singleton type of the kind \hask{Nat}.
...
\item For operators, we prefix with \hask{%}: \hask{(%+)} is the singleton for the type-level \hask{+}.
\end{enumerate}
\end{document}
